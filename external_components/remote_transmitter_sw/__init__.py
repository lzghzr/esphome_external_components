...
async def to_code(config):
    pin_a_ = await cg.gpio_pin_expression(config[CONF_PIN_A])
    pin_b_ = await cg.gpio_pin_expression(config[CONF_PIN_B])
    if CORE.is_esp32:
        var = cg.new_Pvariable(config[CONF_ID], pin_a_)
        ...
        if CONF_EOT_LEVEL in config:
            ...
            cg.add(
                var.set_eot_level(
                    config[CONF_PIN_A][CONF_MODE][CONF_OPEN_DRAIN]
                    or config[CONF_PIN_A][CONF_INVERTED]
                )
            )
    else:
        var = cg.new_Pvariable(config[CONF_ID], pin_a_)
...
FILTER_SOURCE_FILES = filter_source_files_from_platform(
    {
        "remote_transmitter_sw_esp32.cpp": {
